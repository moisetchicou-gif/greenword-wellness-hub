...
const TrustItem = ({ item, index }: { item: typeof trustItems[0]; index: number }) => {
  ...
  return (
    <div
      ref={ref}
      className={`flex items-center gap-4 justify-center transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
      style={{ transitionDelay: `${index * 120}ms` }}
    >
      <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center shrink-0 hover-lift">
